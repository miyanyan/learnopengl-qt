...
void CoordinateSystems::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/1.getting_started/6.1coordinate_systems/CoordinateSystems.vert");
        if(!result){
            qDebug() << m_program.log();
        }
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/1.getting_started/6.1coordinate_systems/CoordinateSystems.frag");
        if(!result){
            qDebug() << m_program.log();
        }
