...
void BasicLightingDiffuse::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/2.lighting/2.1basic_lighting_diffuse/basic_lighting.vert");
        if(!result){
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/2.lighting/2.1basic_lighting_diffuse/basic_lighting.frag");
        if(!result){
            qDebug() << m_lightShader.log();
        }
