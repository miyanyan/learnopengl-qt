...
void LightingMapsDiffuseMap::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/2.lighting/4.1lighting_maps_diffuse_map/lighting_maps_diffuse_map.vert");
        if(!result){
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/2.lighting/4.1lighting_maps_diffuse_map/lighting_maps_diffuse_map.frag");
        if(!result){
            qDebug() << m_lightShader.log();
        }
