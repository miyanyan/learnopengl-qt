...
void LightingMapsExercise2::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/2.lighting/4.3lighting_maps_exercise2/lighting_maps_exercise2.vert");
        if(!result){
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/2.lighting/4.3lighting_maps_exercise2/lighting_maps_exercise2.frag");
        if(!result){
            qDebug() << m_lightShader.log();
        }
