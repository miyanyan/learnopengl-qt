#include "FaceCullingExercise1.h"

#include <QDebug>
#include <QMessageBox>

FaceCullingExercise1::FaceCullingExercise1(QWidget *parent)
...
FaceCullingExercise1::~FaceCullingExercise1()
{
    makeCurrent();
...
void FaceCullingExercise1::initializeGL()
{
    if(!context()){
    ...
    {
        bool result = true;
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/4.advanced_opengl/1.1depth_testing/depth_testing.vert");
        if(!result){
            qDebug() << m_program.log();
        }
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/4.advanced_opengl/1.1depth_testing/depth_testing.frag");
        if(!result){
            qDebug() << m_program.log();
        }
...
void FaceCullingExercise1::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void FaceCullingExercise1::paintGL()
{
...
bool FaceCullingExercise1::event(QEvent *e)
{
    m_camera.handle(e);
