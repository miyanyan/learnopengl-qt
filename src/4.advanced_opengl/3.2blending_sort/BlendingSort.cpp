...
BlendingSort::BlendingSort(QWidget *parent)
...
void BlendingSort::initializeGL()
{
    if(!context()){
    ...
    {
        bool result = true;
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/4.advanced_opengl/3.2blending_sort/blending_sort.vert");
        if(!result){
            qDebug() << m_program.log();
        }
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/4.advanced_opengl/3.2blending_sort/blending_sort.frag");
        if(!result){
            qDebug() << m_program.log();
        }
...
void BlendingSort::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void BlendingSort::paintGL()
{
...
bool BlendingSort::event(QEvent *e)
{
    m_camera.handle(e);
